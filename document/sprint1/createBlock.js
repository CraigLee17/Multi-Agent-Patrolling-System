...
function maps() {
    ...
    for (var i = 0; i < areas; i++) {
        ...
        if (algorithm == "Free Form" && (newAgents[i].length < 1 || newAgents[i].length > Math.ceil(regionArr[i].length / 2))) {
			...
			var nodeSearch1 = 0;
			while(endPointCounter != 1 && nodeSearch1 < regionArr[i].length){//search through region until an endpoint is found
				endPointCounter = 0;
			...
			if(endPointCounter == 1){//if there are endpoints, make sure agents are placed at endpoints
				
				for (var agentSearch = 0; agentSearch < newAgents[i].length; agentSearch++) {
...
function getAllRegionTargetListForSpecifiedStep() {
    for (var i = 0; i < allTargetList.length; i++) {
        var coordinates = "";
        for (var j = 0; j < regionArr[i].length; j++) {
            if (algorithm == "Free Form") {
				if(regionArr[i][j][2] == false){
